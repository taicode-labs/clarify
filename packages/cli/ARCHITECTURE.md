...
| 职责 | 当前问题 | 正确归属 |
|------|----------|----------|
| MDX 编译 | 不应在 core 中编译 MDX。实际的 MDX→JS 编译由 Vite bridge 的 `@mdx-js/rollup` 插件完成；Core 仅做一次**轻量诊断编译**（见 §8.1）以提前暴露语法错误。 | Vite bridge / `renderer/server` |
| Shiki 高亮 | 不应在 core 中执行代码高亮。Shiki 只在 Vite 构建管线的 rehype 阶段运行；Core 的诊断编译显式跳过 `rehypePlugins`。 | Vite bridge / `renderer/server` |
...
- **诊断编译跳过 rehype 管线。** `findContentRoutes` 调用 `compileMdxContent` 提前暴露 MDX/JSX 语法错误，但该调用**不传 `rehypePlugins`**（含 Shiki 高亮、slug、code-block 解析）。真正的编译与高亮在 Vite 构建时由 `@mdx-js/rollup` 插件执行一次。不要在诊断路径重新挂载 rehype 插件，否则每个 `.mdx` 会在每次 `engine.refresh()` 时跑两遍完整编译 + Shiki。
