...
function canonicalUrl(contextOrConfig: ClarifyProjectContext | ResolvedProjectConfig, route: ContentRoute): string | undefined {
  const projectConfig = resolveProjectConfigFromContext(contextOrConfig)
  const alternates = getContentRouteAlternates(route)
  const locale = getContentRouteLocale(route)
  if (!projectConfig.siteUrl || !alternates || !locale) return undefined
  const prefixedPath = alternates[locale]
  if (!prefixedPath || prefixedPath === getContentRoutePath(route)) return undefined
...
export function isNotFoundRoute(route: ContentRoute): boolean {
  return (getContentRouteBasePath(route) ?? getContentRoutePath(route)) === '/404'
}
...
export function routeOutputFiles(outputDirectory: string, route: ContentRoute): string[] {
  ...
  if (isNotFoundRoute(route) && (!getContentRouteLocale(route) || getContentRoutePath(route) === '/404')) {
