...
describe('findContentRoutes', () => {
  ...
  it('uses the document as the canonical source of route identity', () => {
    const route = {
      kind: 'mdx',
      path: '/legacy',
      title: 'Legacy',
      filePath: '/tmp/legacy.mdx',
      virtualModuleId: 'virtual:clarify-page/legacy',
      document: {
        id: '/guide',
        title: 'Guide',
        source: '/tmp/guide.mdx',
        content: [],
        metadata: {},
        route: {
          path: '/guide',
          title: 'Guide',
          filePath: '/tmp/guide.mdx',
          basePath: '/guide',
          locale: 'en-US',
          isFallback: true,
          isBareAlias: false,
          alternates: { 'zh-CN': '/zh-CN/guide' },
          virtualModuleId: 'virtual:clarify-page/legacy',
        },
      },
    }

    expect(getContentRouteIdentity(route)).toEqual({
      path: '/guide',
      title: 'Guide',
      filePath: '/tmp/guide.mdx',
      basePath: '/guide',
  ...
  it('keeps invalid MDX syntax as renderer-handled markdown content', async () => {
    writeFileSync(join(tempDir, 'broken.mdx'), '# Hello\n\n<Thing', 'utf-8')

    const result = await findContentRoutes(tempDir)

    expect(result[0].document?.metadata.diagnostic).toBeUndefined()
    expect(result[0].document?.content).toEqual([{ kind: 'markdown', value: '# Hello\n\n<Thing' }])
  })
