...
| `logo` | 品牌 Logo 路径。 |
| `base` | 站点的 base URL。可被插件配置覆盖。 |
| `theme` | 主题配置（Phase 1 仅支持 `primary` 颜色）。 |
| `openApi` | OpenAPI 规范文件路径。 |
...
| `base` | 部署路径，覆盖 `clarify.json` 中的设置。 |
...
// vite.config.ts
clarifyPlugin({
  base: '/staging/docs/', // 覆盖 clarify.json 以适配 staging 构建
});
```
...
- **错误**：无效的 `base`（未以 `/` 开头）、`docsRoot` 路径不存在、`nav` 结构格式错误。
...
| "如何指向我的 OpenAPI 规范？" | `clarify.json` |
