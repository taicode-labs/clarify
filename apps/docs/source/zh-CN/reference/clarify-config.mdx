...
| `theme` | `{ preset?: 'default' \| 'mint' \| 'violet'; primary?: string }` | `{ preset: 'default' }` | 主题预设和基于预设的局部覆盖 |
...
```ts
theme: {
  preset?: 'default' | 'mint' | 'violet'
  primary?: string
}
```

`preset` 用来选择内置主题基底。当前提供三个预设：

| 预设 | 主色 | 适用场景 |
|------|------|----------|
| `default` | `#0ea5e9` | 默认蓝色科技感文档站 |
| `mint` | `#10b981` | 更轻盈、偏产品或教程类文档 |
| `violet` | `#8b5cf6` | 更强调品牌感或开发者平台风格 |

剩余字段会在预设基础上覆盖。例如下面配置会先应用 `mint`，再把主色覆盖为品牌色：
