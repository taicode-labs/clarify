...
| `themeEditor` | `boolean` | `false` | 是否在构建后的站点中暴露实时主题编辑器；dev 模式会自动启用 |
...
主题 token 是“局部覆盖”而不是重写整套主题；没有配置的字段会继续继承所选 preset 的默认值。当前版本不会读取用户项目中的自定义 CSS 或 `tailwind.config.js` 来覆盖渲染器样式。
