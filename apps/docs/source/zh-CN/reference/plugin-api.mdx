...
| `slots` | `ClarifyUISlotRegistration[]` | 否 | 注册的 UI 插槽组件 |
...
```ts
type ClarifyUISlotName = 
  | 'page.footer.before'       // 页脚之前
  | 'page.banner.replace'      // 替换顶部横幅
  | 'page.footer.replace'      // 替换页脚
```

### `ClarifyUISlotRegistration`

注册一个插槽组件：

```ts
type ClarifyUISlotRegistration = {
  name: ClarifyUISlotName      // 目标插槽位置
...
export default function CustomBanner() {
  ...
  const { DefaultComponent } = useClarifySlot()
