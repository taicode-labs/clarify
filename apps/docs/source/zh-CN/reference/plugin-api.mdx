...
type UISlotRegistration = {
  ...
  component: string            // React 组件模块路径（使用 / 前缀指向项目根目录，如 /source/plugins/MyFooter.tsx）
}
```
...
Hook 可以同步或异步返回。`pages:resolved`、`routes:resolved` 和 `modules:before` 需要返回与输入同类型的结果，`build:done` 无需返回值。
