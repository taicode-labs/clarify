...
Clarify 支持在 `tabs[].pages` 内嵌套 `group`，这样可以表达更深的侧边栏结构，而不会把导航扁平化。
          ...
          pages: [
            {
              ...
              icon: 'Wrench',
              pages: [
                { page: 'guides/install', title: '安装', icon: 'Package' },
                { page: 'guides/configure', title: '配置', icon: 'SlidersHorizontal' },
              ],
            },
            {
              group: '高级',
...
页面引用使用与文件路由相同的路径语法，相对于 `source/`。

例如：`source/guides/advanced/ssg.mdx` 对应 `/guides/advanced/ssg`。
