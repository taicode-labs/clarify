...
```
├── apps/
│   ├── docs/           # Documentation playground & development site (port 5173)
│   └── www/            # Marketing website & landing pages (port 5174)
├── packages/
│   ├── renderer/       # Shared React components & UI primitives
│   └── cli/            # Clarify CLI and documentation build engine
```
...
| `ContentRoute` | CLI → Renderer | A single renderable route, including path, locale, title, description, file path, virtual module, headings, and `contentArtifactUrl`. |
