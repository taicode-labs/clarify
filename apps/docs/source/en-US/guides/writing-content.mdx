...
- Nested directories map directly to nested routes, for example `source/guides/advanced/ssg.mdx` becomes `/guides/advanced/ssg`, and `source/guides/advanced/index.mdx` becomes `/guides/advanced`.
- If the site is deployed under a subpath, configure the prefix with `routePrefix` — see [Configuring the Site](/guides/navigation).

## Nested directories in sidebar config

When you explicitly configure `tabs[].pages`, use the same nested page path syntax to point to deeper files:
      ...
      pages: [
        {
          group: 'Guides',
          pages: [
            { page: 'guides/advanced/ssg', title: 'SSG' },
          ],
        },
      ],
    },
...
This keeps the sidebar structure aligned with the file tree and route paths.
