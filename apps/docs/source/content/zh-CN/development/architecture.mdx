...
- **关键特性**：
  ...
  - 通过 `@clarify/cli` 进行 MDX/OpenAPI 编译、路由、开发服务器和静态构建
  - 为 CLI 和渲染器变更提供真实的测试环境
- **依赖**：`@clarify/renderer` (workspace), `@clarify/cli` (workspace)
- **构建输出**：作为官方文档部署的静态站点
...
- **分发**：发布为 `@clarify/cli`，暴露 `clarify` bin
...
2. **`@clarify/cli`**扫描内容目录，编译 MDX，摄取 OpenAPI，并生成路由
...
文档站点通过 `@clarify/cli` 启动，普通用户无需维护构建工具配置。你可以运行：
