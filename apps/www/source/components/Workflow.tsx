  ...
  return (
    <section id="workflow" className="border-y border-zinc-900/10 bg-zinc-50 px-6 py-20 lg:px-8 dark:border-white/10 dark:bg-zinc-900/30">
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_31rem] lg:items-center">
        <div>
          <p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-emerald-500 dark:text-emerald-400">{t('workflow.eyebrow')}</p>
          <h2 className="mt-4 max-w-2xl text-3xl font-semibold tracking-tight text-zinc-900 sm:text-4xl dark:text-white">{t('workflow.title')}</h2>
          <div className="mt-10 space-y-8">
            {workflowSteps.map(([title, description], index) => (
              <div key={title} className="flex gap-4">
                ...
                <div>
                  <h3 className="text-base font-semibold text-zinc-900 dark:text-white">{t(title)}</h3>
                  <p className="mt-1 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{t(description)}</p>
                </div>
              </div>
            ))}
          </div>
